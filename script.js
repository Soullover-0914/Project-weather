document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Tone.js for Thunder Sound ---
    // Removed Tone.js initialization and related audio logic as per user request.
    // If you wish to re-add sound, you'll need to re-introduce Tone.js setup here.
    ...
    const criticalElements = {
        ...
        voiceInputButton, weatherAlertBox, alertMessageElem, closeAlertButton
    };
...
function speakWeatherSummary(currentData, resolvedCityName) {
    ...
    function displayWeatherAlert(message) {
        if (weatherAlertBox && alertMessageElem) {
            alertMessageElem.innerHTML = message; // Use innerHTML to allow <br> tags
    ...
    function checkAndDisplayAlerts(current) {
        ...
        switch (weatherMain) {
            case 'Thunderstorm':
                alertMessages.push("⚡️⛈️ **Severe Thunderstorm Warning!** Expect heavy rain, strong winds, and frequent lightning. Seek sturdy shelter immediately. Avoid windows and unplug sensitive electrical appliances. Do not go outside.");
                break;
            case 'Rain':
            case 'Drizzle':
